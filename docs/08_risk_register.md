...
| EKS PoC 범위 확대                      | 일정 지연과 비용 증가            | 중     | EKS PoC는 생성, 샘플 앱 배포, 삭제 검증 수준으로 제한하고 운영용 전환은 향후 과제로 분리                |
| ALB 용어 혼동                          | 구현 방향 혼선                   | 중     | MVP ALB는 Terraform ALB/Target Group, AWS Load Balancer Controller는 EKS 확장 기능으로 구분             |
