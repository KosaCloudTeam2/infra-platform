...
확장으로 분리함. ECR은 AWS-only fallback 또는 AWS 배포를 강하게 묶어야 할 때의 대체안으로 유지함.
...
- 기존 ECR 기반 Terraform과 workflow는 AWS-only fallback 또는 후속 전환 대상으로 유지함

## 후속 작업

- GitHub Actions Docker Hub push workflow 정리
- Kubernetes manifest image 경로를 Docker Hub 기준으로 정리
