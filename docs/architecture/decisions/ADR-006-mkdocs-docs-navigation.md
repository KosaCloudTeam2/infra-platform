...
`docs/` 바로 아래에 문서가 많아지고 있어 웹 문서 형태의 탐색성이 필요함. 하지만 현재 `00~20` 문서는
처음 읽는 순서를 표현하고 있으므로, 물리적으로 폴더를 크게 이동하면 링크 수정과 혼란이 커질 수 있음.

## 결정

문서 파일은 현재 구조를 유지하고, `docs/index.md`와 `mkdocs.yml`의 `nav`로 읽기 순서와 영역별 탐색을
제공함. MkDocs Material을 사용하고 로컬 미리보기는 `127.0.0.1:8010` 포트를 사용함.
...
- MkDocs navigation으로 온보딩 순서와 역할별 문서를 함께 표현 가능
