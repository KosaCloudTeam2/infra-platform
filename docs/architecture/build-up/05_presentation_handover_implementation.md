...
구현 결과를 발표 가능한 이야기로 정리하고, 발표 후 리소스 정리와 인계가 가능한 상태로 문서와
산출물을 마감함.

## 2. 사전 조건

- 각 담당자가 구현 결과, 검증 결과, 캡처를 제공함
...
2. Network/IaC 담당 캡처 수집
3. DB/Storage 담당 캡처 수집
4. CI/CD/App Runtime 담당 캡처 수집
5. Observability/Demo 담당 캡처 수집
...
| 담당               | 필수 자료                                          |
| :----------------- | :------------------------------------------------- |
| Network/IaC        | ALB DNS, Subnet/SG 캡처, Terraform plan 요약       |
| DB/Storage         | PXC 상태, ProxySQL endpoint, Ceph 백업 경로        |
| CI/CD/App Runtime  | GitHub Actions 실행 결과, ECR 태그, ECS deployment |
| Observability/Demo | CloudWatch Logs/Alarm 캡처, 장애 시나리오 결과     |
