...
1. `terraform.tfvars.example` -> `terraform.tfvars` 복사
2. 아래 항목 최소 확정
   - 네트워크: `vpc_cidr`, `public_subnet_*`, `az_*`
   - 접근: `admin_cidr`, `key_name`
   - 경로 A (S2S VPN): `create_site_to_site_vpn`, `customer_gateway_public_ip`,
     `customer_gateway_bgp_asn`, `onprem_cidr`
   - DNS: `create_route53_zone`, `domain_name`, `create_route53_alias_record`, `app_fqdn`

> 각 변수의 의미/실측 확인 명령은 `prerequisites` 가이드를 기준으로 함.
