...
기존 애플리케이션을 온프레미스 Kubernetes에서 기본 운영하고, 부하 증가 시 AWS EC2 Auto Scaling/ALB
burst 영역으로 확장하는 비용 우선 하이브리드 인프라 플랫폼 구축 프로젝트

## 2. 성공 기준

- GitHub `main` 브랜치에 병합된 코드가 GitHub Actions를 통해 Docker 이미지로 빌드됨
- 빌드된 이미지가 ECR 또는 팀 표준 컨테이너 레지스트리에 태그와 함께 저장됨
...
- WAF 또는 Security Group으로 최소 보안 정책이 적용됨
- AWS RDS 없이 Percona XtraDB Cluster와 ProxySQL 기반 DB 접근이 가능함
- Percona XtraBackup 결과를 Ceph RGW에 저장할 수 있음
