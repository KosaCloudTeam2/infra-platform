...
이번 반영에서 문서 탐색성을 높이기 위해 아키텍처 문서를 아래처럼 단계 분리함.

- Level 0(통합): `docs/01_architecture.md`
- Level 1(영역 상세): `docs/architecture/details/*.md`
- Level 2(운영 시퀀스 연결): `docs/architecture/details/ops_flow_and_extensions.md` +
  `docs/runbooks/*`

적용 상태:

- Phase 1: Level 0/1/2 분리 및 전환 안내 적용 완료
- Phase 2: `docs/01_architecture.md` 통합 개요 중심 슬림화 완료

적용 원칙:

- 2026-05-16까지 1주 동안 레거시 탐색 호환 안내를 유지함
- Mermaid 정본은 기술 문서(`docs/01_architecture.md`, `docs/architecture/details/*`,
  `docs/architecture/build-up/*`)에 둠
- 문서 검증 필수 게이트는 `uv run mkdocs build` 단일 기준으로 운영함
