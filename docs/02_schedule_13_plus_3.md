...
| B. Cloud / Network / IaC Engineer     | 팀원 2 | VPC, Subnet, ALB, SG, WAF, EC2 ASG, Launch Template, IAM/OIDC, 비용 제한, 온프레 DB 연동 경계(클라우드 네트워크 범위), EKS 최소 PoC 주 담당 |
...
| Day 2  | 설계 확정                   | 아키텍처 다이어그램 확정, 역할별 작업 보드 구성                       | Terraform Backend/Provider 구조 작성, App Subnet 경계 설계     | DB VM 사양, Ceph RBD 풀, 백업 저장소 요구사항 제시                                   | OIDC 배포 방식 설계, 앱 포트/헬스체크 경로 확인                            |
| Day 3  | 네트워크 골격               | 설계 리뷰 및 리스크 점검                                              | VPC, Public/App Private Subnet, IGW, Route Table 구현          | PXC 설치 절차와 ProxySQL 라우팅 초안 작성                                            | GitHub Actions 기본 workflow 작성                                          |
...
| Day 5  | 첫 배포 준비                | MVP 범위 재점검, 일정 조정                                            | AWS burst 경계(ALB/SG/경로) 검증                               | DB용 Proxmox VM, Ceph RBD 연결, PXC/ProxySQL 설치 준비, Ceph RGW 접근 방식 확정      | Docker Hub push 권한, GitHub Actions Secret 검증                           |
| Day 6  | 첫 배포 성공                | 첫 배포 체크리스트 운영                                               | ALB Health Check 경로와 SG 수정                                | DB VM 접속 경로(Bastion/VPN) 검증                                                    | 온프레미스 K8s 첫 배포, GitHub Actions 이미지 빌드, Argo CD 설치 착수      |
...
| Day 8  | 데이터 계층                 | DB/Ceph 시연 범위 확정, 웹 서버 기동/접속과 앱-DB 연결 검증 기준 확정 | AWS burst→온프레 DB 접근 경계 검토                             | PXC 3노드, ProxySQL 1대 MVP 구성 완료 목표                                           | Argo CD Application 구성/sync, 앱 배포 산출물 준비                         |
...
| Day 13 | 비용/성능 정리 및 구축 마감 | 최종 산출물 점검, 발표 핵심 메시지 작성, 누락 항목 결정               | ALB, EC2 burst 비용 추정 정리, IaC README 정리                 | 온프레 DB VM, ProxySQL, Ceph 운영 기준과 명령 정리                                   | WAF/CloudWatch/CI/CD 비용과 보안 효과 정리, 로그 캡처 정리                 |
...
- 웹 서버 기동 확인, Ingress/ALB 접속 검증, 앱-DB 연결 검증 주관
- Runbook 품질 점검
...
- App Private Subnet 경계와 온프레 DB 접근 경로 분리
- DB용 Proxmox VM/Ceph RBD 구성 기준과 온프레 관리 접속 경로
...
- DB 관련 온프레미스 네트워크(pfSense/VLAN/라우팅/방화벽) 기준 정리
- DB용 Proxmox VM 사양, Ceph RBD 디스크, 패키지 요구사항 정의
