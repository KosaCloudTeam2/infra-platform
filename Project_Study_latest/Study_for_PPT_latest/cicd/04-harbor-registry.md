...
A. Harbor UI에서 project 생성 시 "Project Access Level: Private"를 선택합니다. 그 project의 이미지는 **robot account나 admin만 pull 가능**합니다. ticket-app은 현재 `library/kosa-tickets`라 public이지만 imagePullSecret을 사용하는 패턴으로 운영 중입니다 (향후 private 변경 시 코드 변경 없음).
