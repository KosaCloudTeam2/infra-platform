...
> ⭐ **한 줄 요약**: **git push → Jenkins polling 감지 → Kaniko build → Harbor push → kosa-gitops image tag 갱신 → ArgoCD sync → Pod rolling update**. 평균 5~10분에 코드가 production에 반영된다. 각 컴포넌트가 명확한 역할을 가지고 chain으로 동작한다.
