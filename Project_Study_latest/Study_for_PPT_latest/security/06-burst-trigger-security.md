...
> ⭐ **한 줄 요약**: 현재 **AlertManager → Lambda webhook이 무인증**이다 (학습 환경). 누구나 URL 알면 false burst를 발사할 수 있는 위험이 있고, Phase 6에서 **API Gateway API Key 또는 HMAC 인증**을 추가하는 게 우선이다.

---

## 🎯 현재 상태

Burst trigger는 우리 하이브리드 인프라의 자동화 핵심인데, 그 인증이 빠져있다. AlertManager가 발사하는 webhook을 누구든 호출할 수 있는 상태다.
...
URL은 학습 환경에서 git commit이나 helm values에 노출돼 있어 사실상 공개 상태다. 진짜 운영급이면 이게 큰 보안 위험이다.
