...
> ⭐ **한 줄 요약**: **Percona XtraDB Cluster (PXC) 3-node**가 primary DB로 Galera 동기 복제를 한다 (Percona Operator로 자동 관리). **Redis Sentinel 3-node**가 캐시/세션/queue를 담당. 둘 다 K8s 위에서 HA를 구현하고 anti-affinity로 강제 분산된다. **온프레와 AWS EKS 양쪽이 같은 PXC + Redis를 공유** (전용선 통해) — single source of truth.
...
| MetalLB 노출 | `kosa-redis-{0,1,2}-svc` (172.16.23.57/58/59) — EKS 공유용 |

### 온프레-AWS 양쪽이 같은 데이터 layer 공유 ⭐

burst 시점에 EKS Pod이 들어오는데, 이 Pod도 **온프레 PXC + Redis**를 그대로 쓴다. 별도 캐시/DB를 AWS에 두지 않는다.

```
[온프레 ticket-app] ─┐
                     ├─→ 온프레 ProxySQL → PXC (write/read)
                     ├─→ 온프레 Redis master (172.16.23.59)
                     │
[AWS EKS ticket-app] ┘   (전용선 통해, single source 공유)
```

왜 공유하나? 세 가지 이유:

**1. 캐시 일관성** — 사용자 X가 좌석 예약하면 Redis DEL이 발생한다. EKS가 별도 Redis를 쓰면 invalidate 신호 못 받아 stale cache 위험. 단일 Redis면 모든 client가 같은 상태.

**2. 단일 source of truth** — write가 PXC 한 곳에 모이니 정합성 보장. 양쪽에 write 받으면 conflict.

**3. 운영 단순함** — 캐시 한 벌, DB 한 벌. 모니터링/backup/failover 한 곳만 보면 됨.

검증된 결과 (2026-05-23):
- EKS Pod의 `/api/users/25` 첫 호출: cache MISS, PXC→Redis 273ms
- 두 번째 (다른 EKS Pod에서 호출): cache HIT, Redis만 6.6ms
- **다른 EKS Pod에서도 같은 cache hit** → cluster-wide 공유 확인

### EKS 노출용 LoadBalancer

bitnami Redis chart가 ClusterIP만 만들어서 EKS에서 직접 못 봄. 그래서 노드별 LB IP 노출:

| Service | IP | 역할 |
|---|---|---|
| `kosa-redis-0-svc` | 172.16.23.57 | node-0 (현재 slave) |
| `kosa-redis-1-svc` | 172.16.23.58 | node-1 |
| `kosa-redis-2-svc` | 172.16.23.59 | **현재 master** ⭐ |

ProxySQL도 LoadBalancer로 노출:
| Service | IP | 역할 |
|---|---|---|
| `ticket-proxysql` | 172.16.23.56 | ticket-app 전용 ProxySQL (LB) |
| `proxysql-bridge` | 172.16.23.55 | RDS binlog 전송용 bridge |

### EKS의 aws-ticket-proxysql

EKS 내부에 별도 `aws-ticket-proxysql` Pod이 있다. 이건 EKS Pod이 온프레 ProxySQL과 직접 연결하기 전에 connection pool을 잡아주는 layer다.

```
EKS ticket-app
  ↓ (cluster 내부)
aws-ticket-proxysql (EKS, ClusterIP)
  ↓ hostgroup 10 (single)
전용선/VPN
  ↓
172.16.23.56:6033 (온프레 ticket-proxysql)
  ↓
PXC writer/reader
```

ConfigMap의 `mysql_servers`에 hostgroup 10만 두고 RDS hostgroup (이전 20)을 제거함. **모든 query가 온프레 PXC로 가도록 통일**.
