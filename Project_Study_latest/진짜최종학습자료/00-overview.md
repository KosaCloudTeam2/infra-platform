...
> ⭐ **한 줄 요약**: 온프레미스(Proxmox+Ceph+K8s) + AWS 하이브리드. 평시 100% 온프레, 트래픽 급증 시 Burst로 EKS에 분산한다. **데이터 layer는 운영(OLTP)/분석(OLAP) 분리** — 운영은 온프레 PXC+Redis (EKS도 공유), 분석은 AWS RDS Read Replica로 격리. 4명 분담, GitOps 기반, 관측 3대 축 (Metrics+Traces+Logs).
...
```
        ...
        │     │                       │    │     │                    │
        │     ▼                       │    │     ▼                    │
        │  Pods (kosa-tickets, admin) │    │  EKS + Karpenter Spot    │
        │     │                       │    │  (평시 0, burst시 자동)  │
        │  ┌──┴──────┐                │    │     │                    │
        │  │ PXC      │ ◄──────────── │ ───┤  EKS ticket-app          │
        │  │ Redis    │ ◄────공유──── │ ───┘     (단일 source 공유)   │
        │  │ Ceph     │               │                                │
        │  └──────────┘               │    ┌──────────────────────────┐│
        │  3-replica, 10G fabric      │    │ OLAP 분리 path            ││
        │       ▲                     │    │   RDS Read Replica        ││
        │       │ binlog              │    │   (admin-app, Grafana 전용)││
        │       │ via VPN/전용선      │    └──────────────────────────┘│
        └───────┼─────────────────────┘                                 │
                │                                                       │
                │           Site-to-Site VPN (전용선 가정)              │
                └────────────────IPsec, ~6ms latency────────────────────┘

  Burst Trigger:
   Prometheus → AlertManager → Lambda → R53 weight 변경 → 트래픽 AWS로
   (EKS Pod도 온프레 PXC + Redis 그대로 사용 — single source of truth)

  데이터 layer 분리 (OLTP vs OLAP):
   운영 path (burst 포함) → 온프레 PXC + Redis (단일 source)
   분석 path → AWS RDS Read Replica (격리, admin/Grafana 전용)
```

핵심은 두 가지다. **첫째, Route 53이 트래픽을 분배** — 평시 온프레 70 / AWS 30, burst 시 AWS 비율 ↑. **둘째, 데이터 layer는 OLTP/OLAP 분리** — 운영(burst 포함)은 온프레 single source, 분석은 RDS Replica로 격리해서 무거운 분석 쿼리가 OLTP에 영향 0.
...
운영 트랜잭션(OLTP)은 온프레 PXC + Redis, 분석 쿼리(OLAP)는 AWS RDS Read Replica로 **물리적으로 분리**. EKS burst Pod도 운영 path는 온프레 single source 공유. 무거운 분석 쿼리(매출 집계, 점유율 통계)가 운영 DB를 마비시키지 않음 = 운영급 회사의 정석 패턴.

---

## 📖 자주 등장하는 용어 (3단어)

이 프로젝트 전체에서 자주 나오는 핵심 용어 3개. 자세한 건 각 파트 문서 참고.

### OLTP (Online Transaction Processing)

**"운영 트랜잭션"**. 사용자가 클릭할 때마다 발생하는 짧고 빠른 query (1~10ms). 예: 좌석 1개 조회, 예약 1건 INSERT. 동시성 높음 (초당 수천 query). 우리는 **온프레 PXC**가 담당.

### OLAP (Online Analytical Processing)

**"분석 집계"**. 관리자/분석가가 보는 무거운 집계 query (1초~수분). 예: 월별 매출, 점유율 TOP 10, 시간대별 패턴. 동시성 낮음 (분당 수 query). 우리는 **AWS RDS Read Replica**가 담당.

### 왜 분리?

같은 DB에 두 워크로드를 섞으면 **무거운 OLAP query 하나가 가벼운 OLTP query 수천 개를 30초 멈추게** 한다. 분리하면 서로 영향 0. 운영급 회사 데이터 계층의 정석 패턴. 자세한 5가지 정당화는 `data-storage/06-rds-replication.md`.
