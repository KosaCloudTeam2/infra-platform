...
워크로드를 namespace로 분리해서 NetworkPolicy, RBAC, 리소스 쿼터를 깔끔히 적용한다. **OLTP / OLAP 분리**는 namespace 레벨에서도 적용.

| Namespace | 용도 | nodeSelector | DB 접근 |
|---|---|---|---|
| `kosa-tickets` | 운영 ticket-app + EKS ProxySQL | production | ProxySQL → PXC |
| `pii-protected` | PXC + ProxySQL (PII 데이터) | production | 자체 |
| `redis` | Redis Sentinel | production | 자체 |
| `monitoring` | Prometheus/Grafana/Tempo/Loki | system | RDS (분석용) ⭐ |
| `admin` ⭐ | admin-app (관리자 대시보드) | system | **RDS Read Replica 직결** (ProxySQL 우회) |
| `harbor` | Container registry | system | Harbor 자체 DB |
| `jenkins` | CI 서버 | system | 자체 |
| `argocd` | GitOps | system | 자체 |
| `cert-manager` | TLS 자동화 | system | 자체 |

### admin namespace의 의미

기존 `kosa-tickets` (운영) namespace와 **`admin` (분석)** namespace를 명확히 분리. NetworkPolicy로 강제:

- `admin` ns Pod → **RDS Replica만** 접근 가능 (PXC 차단)
- `kosa-tickets` ns Pod → **PXC만** 접근 가능 (RDS 차단)

이러면 admin-app의 코드에 버그/SQL injection이 있어도 운영 DB(PXC)에 영향 0. **OLTP/OLAP 격리의 물리적 강제**.

자세한 NetworkPolicy 정의는 `security/03-network-policy.md`.
