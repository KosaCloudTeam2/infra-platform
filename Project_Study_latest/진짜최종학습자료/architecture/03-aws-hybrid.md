...
| **RDS Read Replica** | kosa-rds-replica (10.20.10.54) | **OLAP 전용** (운영 path와 분리) |

### 데이터 layer — 운영(OLTP) vs 분석(OLAP) 분리

AWS 측 RDS의 역할은 명확히 **OLAP 전용**이다. EKS burst Pod의 운영 트래픽은 RDS를 안 거치고 전용선/VPN을 통해 온프레 PXC + Redis로 간다 — single source of truth 유지.

```
운영 path (OLTP):
  EKS ticket-app → 전용선 → 온프레 PXC + Redis  (RDS 안 거침)

분석 path (OLAP):
  admin-app, Grafana → RDS Read Replica         (별도 격리)
```

자세한 정당화는 `data-storage/06-rds-replication.md`. burst path 상세는 `architecture/04-burst-architecture.md`.
...
[bastion 172.16.24.10]                 [Public Subnet 2a/2c]
  ...
  PXC (pii-protected)                    NLB
  Redis (172.16.23.59 master)              │
        │                                [Private Subnet 2a/2c]
[pfSense WAN .109]                         EC2 HAProxy × 2
        │ (NAT-T)                          EKS Karpenter Spot (burst 시)
        │                                  RDS Read Replica (10.20.10.54) ← OLAP 전용
[ER605 ISP 125.131.208.229]                  │
        ...
        └─ IPsec VPN ──────────────────────┘
           ...
           ↑ burst path (운영 OLTP):
           EKS Pod ─ 전용선 ─→ 온프레 PXC + Redis

           ↑ 분석 path (OLAP):
           admin-app, Grafana ─→ RDS Read Replica (in VPC)
```
