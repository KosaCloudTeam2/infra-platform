...
> ⭐ **한 줄 요약**: 온프레미스(Proxmox+Ceph+K8s) + AWS 하이브리드. 평시 100% 온프레, 트래픽 급증 시 Burst로 EKS에 분산. 4명 분담, GitOps 기반, 관측 3대 축 (Metrics+Traces+Logs) 완성.

이 문서는 **4명 팀원 공통 reference**. 본인 담당 파트로 들어가기 전에 전체 그림을 잡기 위한 것.
...
각 파트는 `README.md` + 5~8개 deep-dive 문서.
...
> 🔥 **각자 본인 파트 deep-dive + 다른 파트 README는 필독**. 면접/발표에서 cross-cutting 질문 무조건 들어옵니다.
...
- VPN 위에 데이터(PXC→RDS) + 워크로드(Burst) 양방향 흐름
- 단순 "VPN으로 두 사이트 연결" 수준이 아닌 자동화된 burst

### 2. **9-layer cascade 사고 분석 + 예방**
- 2026-05-21 incident: cp1 etcd hiccup → 6분 timeout cascade
- 5개 layer 추적, fix 4개 적용 (etcd auto-compact, HAProxy fall 5, GARP, ARP timeout)
- → 인프라 깊이 있게 이해한 증거

### 3. **GitOps 완성** (App-of-Apps + 자동 sync)
- root-app 1개로 모든 service 부트스트랩
- 모든 변경은 git commit → ArgoCD가 reconcile
- selfHeal로 drift 자동 회복

### 4. **관측 3대 축 완성** (Metrics + Traces + Logs)
- Prometheus + Tempo + Loki
- OpenTelemetry auto-instrumentation (코드 0줄 변경)
- AlertManager SMTP email + AWS Lambda webhook 동시 라우팅

### 5. **이중 TLS (Defense in Depth)**
- 외부(Edge HAProxy) + 내부(HAProxy Ingress) 모두 TLS 종료
- 자체 CA (10년) + cert-manager로 service별 cert 90일 회전
- 내부 트래픽도 wire에서 평문 X
...
면접관/발표자가 가장 좋아하는 것 = **자기 약점 인지 + 개선 계획**.
...
→ 8개 모두 문서화됨 (각각 해당 파트 문서에 detail). **솔직히 + 개선 계획 = 면접 강점**.
...
8. ⭐ `architecture/07-bootstrap-resilience.md` → K8s 의존성 + CI/CD 분리 (Bootstrap 문제)
9. ⭐ `architecture/08-onprem-autoscaling.md` → 온프레 워커 자동 프로비저닝 (Terraform + Ansible)
...
**Q. 왜 100% AWS 가지 않고 하이브리드?**
A. (1) 이미 보유한 온프레 자원 활용 (sunk cost), (2) 클라우드 비용 절감 (평시 워크로드는 온프레, burst만 AWS), (3) 데이터 주권 (PXC가 primary, RDS는 replica), (4) 학습 가치 (양쪽 다 다룸).

**Q. 비용은 얼마나?**
A. 온프레 (전기/감가) 약 ₩30만/월, AWS Phase 1-2 약 $130/월 (VPN+NAT+EC2+NLB), Phase 4 burst는 발생시만. 100% AWS 대비 약 60% 절감 추정. 자세한 건 `architecture/06`.

**Q. 4명이 어떻게 분담했나?**
A. 위의 4파트 분담. 각자 자기 영역 deep + 통합은 cross-cutting 문서로.

**Q. 가장 어려웠던 부분?**
A. 9-layer cascade incident, ArgoCD/Operator reconcile 충돌, OTel auto-instrumentation NetworkPolicy 함정. 모두 문서화됨.
