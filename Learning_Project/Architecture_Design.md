...
### 논리 아키텍처 (Cloud Burst + AWS Edge 결합)
...
```
                  ...
                  ┌─────────────────┴─────────────────┐
                  │                                   │
            평시 100% / 이벤트 70%               이벤트 시만 30%
                  │                                   │
                  ▼                                   ▼
        [HAProxy + Keepalived]                   [AWS NLB]
        외부 VIP (TLS 종료)                      (Static IP, L4)
        rate limit, security                          │
                  │                                   ▼
                  ▼                            [AWS EKS]
        [Onprem K8s]                          (Karpenter Spot)
        CP3 + W3                                      │
                  │                                   ▼
                  ▼                       [HAProxy Ingress] ← 동일
        [HAProxy Ingress]                  ArgoCD ApplicationSet sync
        L7 라우팅 (TLS, host, path)                   │
                  │                                   ▼
                  ▼                            [Pods (Burst)]
        [Service (MetalLB IP)]                        │
                  │                                   │
                  ▼                                   │
        [Pods]                                        │
        FastAPI / Redis / Harbor                      │
                  │                                   │
       ┌──────────┼──────────┐                       │
       ▼          ▼          ▼                       │
   [ProxySQL][Percona PXC × 3][Ceph RGW]            │
              │  Onprem 전용                          │
              │                                       │
              └──── binlog 실시간 복제 ────► [AWS RDS Read Replica]
                                              (Read 트래픽 분산)
...
- **Ingress**: **HAProxy Ingress Controller** (L4+L7 지원, 외부 HAProxy와 통일)
...
### 3.2 AWS EKS 클러스터 + NLB Edge

#### Edge Layer (AWS NLB)

이벤트 시 트래픽이 AWS로 분산될 때 진입점:

```
[Route 53 weighted] ──30% (이벤트 시)─► [AWS NLB]
                                       Static IP
                                       Layer 4 (TCP 443)
                                            │
                                            ▼
...
│                                                          │
│  Ingress: HAProxy Ingress (Onprem과 동일)                │
│    └─ ArgoCD ApplicationSet으로 매니페스트 sync          │
└──────────────────────────────────────────────────────────┘
```
...
- TLS 인증서 자동 발급 (cert-manager 연동)
- Galera SST/IST 자동
- 노드 장애 시 자동 복구
- 백업/복원 (`PerconaXtraDBClusterBackup` CR)
- 노드 추가/제거 (`size` 변경만)
