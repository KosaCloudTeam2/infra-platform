...
def home():
    ...
    return render_template_string(
        ...
        {% block body %}
            ...
            {% for employee in employees %}
                <tr>
                  <td width="100">{% if employee.photo_url %}
                  <img width="50" src="{{employee.photo_url}}" /><br/>
                  {% endif %}
...
def save():
    ...
    print("DEBUG: entered /save")
    print("DEBUG: photo exists =", bool(form.photo.data))

    if form.validate_on_submit():
        print("DEBUG: form validate OK")
        if form.photo.data:
            image_bytes = util.resize_image(form.photo.data, (120, 160))
            print("DEBUG: image_bytes is None? ->", image_bytes is None)
            if image_bytes:
                try:
                    prefix = "employee_pic/"
                    key = prefix + util.random_hex_bytes(8) + ".png"
                    print("DEBUG: uploading key =", key, "bucket =", config.PHOTOS_BUCKET)
                    ...
                    print("DEBUG: S3 put_object OK")
                except Exception as e:
                    print("DEBUG: S3 put_object FAILED:", repr(e))
                    raise
        ...
        try:
            ...
            print("DEBUG: DB write OK")
        except Exception as e:
            print("DEBUG: DB write FAILED:", repr(e))
            raise

        flash("Saved!")
        return redirect(url_for("home"))
    else:
        print("DEBUG: form validate FAILED", form.errors)
...
def view(employee_id):
  ...
  <div class="row">
    <div class="col-md-4">
        ...
        <img alt="Mugshot" src="{{ employee.photo_url }}" />
        {% endif %}
    </div>
...
def photo(employee_id):
    "Proxy image from Ceph S3 through Flask"
    s3_client = get_s3_client()
    employee = database.load_employee(employee_id)
    ...
    try:
        obj = s3_client.get_object(Bucket=config.PHOTOS_BUCKET, Key=employee["object_key"])
        return Response(obj["Body"].read(), mimetype=obj.get("ContentType", "image/png"))
    except Exception as e:
        print("S3 proxy read failed:", repr(e))
        abort(404)
